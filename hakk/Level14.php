...
$curLevel=14; // Vilken level är det här?
...
// Om man har submittat.
	...
	//Check if vaild mysql
		
	$password='/\w*?[\'"](OR|or)[\'"]?\w*?[\'"]?=[\'"]?\w*?(--)?/';//"/^.*?['\"](OR|or)['\"].*?['\"]=['\"].*?(--)?$/i"; // lösenordet är ...
	if(preg_match($password,stripslashes(urldecode($_GET["Pass"]))))
	{  
	// om lösen är rätt
...
$Error="Fel lösenord. PS. Mellanrum är inte tillåtna!";
...
<!-- MySQL Passwordsaving system COPYRIGHT MySQLsaver@Verath.se -->
